...
protected:
  ...
  void initLED() {
    ...
    // Core 3.x: single call binds the pin to a LEDC timer; channel is implicit.
  ...
  #if ESP_ARDUINO_VERSION >= ESP_ARDUINO_VERSION_VAL(3, 0, 0)
    // Core 3.x: ledcWrite() addresses the PIN, not the channel.
    #if BOARD_LED_INVERSE
    ledcWrite(BOARD_LED_PIN, TO_PWM(255 - color));
    #else
